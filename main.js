...
function start() {
    ...
    //  </p>
    // </div>
    var filter = d3.select(graph)
        .append('p')
        .append('button')
        .text('Filter Data')

    //reset Filter
    var reset = d3.select(graph)
        .append('p')
        .append('button')
        .text('Reset Filter')

    //create data = list of groups
    var allGroup = ["steelblue", "green", "brown", "red"]

    var dropdownButton = d3.select(graph).append('p').append('select')
    //colorOption
    ...
    d3.csv('data.csv', function(d) {
        ...
        return d;
    }, function Build(error, data) {
        csv = data;

        //filter the data based on the initial cutOff number
        // var data = csv.filter(function(d) {
            // var sq = d3.select(cutOff).property('value')
            // return d. frequency >== sq;
        // })

        // We now have the "massaged" CSV data in the 'data' variable.
        var bars = svg.append('g').style('fill', 'steelblue');
        ...
        bars.append('g')
            ...
            .append('rect')
            .attr('class', 'bar')
            .attr('x', 30) //distance between bars and the y axis
            .attr('y', function(d) {
            ...
            .attr('height', function(d) {
                ...
                return yScale.bandwidth()*.8;//width of each bar
            })
            // .exit()


            filter.on('click', function() {
                var selectedOption = dropdownButton.property('value')
                var setDecimal = cutOff.property('value')
                applyFilter(setDecimal)
            ...
            function applyFilter(value) {
                ...
                bars
                    ...
                    .attr('height', function(d) {
                        ...
                        // with bands from 0 to height? And then we set the domain
                        // to contain all the letters in the alphabet?
                        return yScale.bandwidth()*.81;//width of each bar
                    })
                    .transition().duration(1000)
                    .attr('width', function(d) {
                        // xScale will map any number and return a number
                        // within the output range we specified earlier.
                        return xScale(d.frequency) * 1.1;
                    })
                    .style('fill', 'white')

                    var showData = csv.filter(function(d) {
                        return d.frequency > +value;
                    })
                    bars
                        .append('g')
            ...
            //function that changes the colorOption
            function updateColor(myColorOptions) {
                 bars.transition()
                        .duration(1000)
                        .style("fill", myColorOptions)
            }
            ...
            //when the reset filter is clicked, run the resetFilter function
            reset.on("click", function(){
                resetFilter()
                updateColor('steelblue')
            })
    });
}
